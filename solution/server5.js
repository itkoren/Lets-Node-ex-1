...
// Sentimentor constructor
var Sentimentor = function() {};
util.inherits(Sentimentor, EventEmitter);

/**
 * Processes an input sentiment score and emits a "sentiment" event
 * @param {String} data - the string to be processed
 * @public
 */	
Sentimentor.prototype.process = function(data) {
  ...
  if (score) {
    // Emit the sentiment event with the score
    this.emit("sentiment", null, score);
  }
};
...
var tcpServer = net.createServer(function(conn) {
  ...
  // Initialize a sentimentor instance
  var sentimentor = new Sentimentor();
  ...
  conn.on("data", function(data) {
    ...
    // Let the sentimentor process the input string
    sentimentor.process(data);
  });
  ...
  // Set an event listener & handler for the sentiment event emitted by the Sentimentor 
  sentimentor.once("sentiment", function(error, score) {
    if (score) {
